...
function Lesson({ lesson }: { lesson: PracticeLesson }) {
  ...
  return <article className="learn-card"><div className="learn-section-head"><div><span className="learn-eyebrow">PRÁTICA {variant === 'british' ? 'BRITÂNICA' : 'AMERICANA'} · {lesson.level}</span><h2>{lesson.title}</h2></div><span className="learn-pill">{state.checks[`lesson-${lesson.id}`]?.[0] ? 'Praticada ✓' : '8–15 min'}</span></div>
    ...
    <button className="learn-button primary" disabled={busy || !checked} onClick={() => save(s => { s.checks[`lesson-${lesson.id}`] = [true]; })}>Marcar esta prática como feita</button><small className="learn-block">Marque depois de ouvir ou ler, conferir e escrever. Isso registra prática, não domínio do nível.</small>
  </article>;
}
...
export function PracticePage() {
  ...
  return <div className="learn-page"><PageHeading eyebrow={`SEU LABORATÓRIO · INGLÊS ${variant === 'british' ? 'BRITÂNICO' : 'AMERICANO'}`} title="Ouvir. Entender. Usar. Lembrar." text={`Oito práticas autorais com vocabulário, ortografia, situações e voz de ${ENGLISH_VARIANTS[variant].shortLabel}. Alterne escuta, leitura e produção; não fique só assistindo.`} />
    ...
    <section className="learn-card" id="revisao"><span className="learn-eyebrow">REVISÃO ESPAÇADA</span><h2>As frases que valem a pena lembrar</h2><p>Tente lembrar antes de revelar a resposta. As frases voltam conforme a dificuldade que você informar.</p><button className="learn-button primary" onClick={async () => { try { setCards(await getDueSrsCards(userId)); setNotice(''); } catch { setNotice('Não foi possível abrir a revisão. Tente novamente.'); } }}>Abrir revisão de hoje</button>{cards && (cards.length ? <SrsReviewTrainer key={cards.map(c => c.id).join('-')} userId={userId} cards={cards} onFinish={() => setCards(null)} /> : <p>Nenhuma frase pendente agora. Você pode guardar uma expressão das práticas acima.</p>)}
