...
export function StudyGuide() {
  ...
  return <div className="learn-page">
    <PageHeading eyebrow={`${variantMeta.label.toUpperCase()} · PARTE POR PARTE`} title="Seu caminho, em três partes." text="Abra uma etapa e pratique no seu ritmo." />
    ...
    {stages.map((stage, index) => <section className="learn-card" id={`parte-${index + 1}`} key={stage.title} style={{ scrollMarginTop: 24 }}><div className="learn-section-head"><div><span className="learn-eyebrow">{stage.subtitle}</span><h2>{stage.title}</h2></div><span className="learn-pill">{completed[index] ? 'Parte percorrida ✓' : `Parte ${index + 1} de 3`}</span></div><details><summary>Ver as 4 atividades desta etapa</summary><ol className="learn-instructions">{stage.steps.map(([title, detail]) => <li key={title}><strong>{title}</strong><p>{detail}</p></li>)}</ol><div className="learn-next"><strong>Quando seguir para o próximo passo?</strong><p>{stage.criterion}</p></div></details><div className="learn-actions">{stage.links.map(([to, label]) => <Link className="learn-button" key={label} to={to}>{label} →</Link>)}</div><label className="learn-check-label"><input type="checkbox" disabled={busy} checked={completed[index] ?? false} onChange={e => { const checked = e.target.checked; void save(s => { const values = [...(s.checks[guideKey] ?? completed)]; values[index] = checked; s.checks[guideKey] = values; }); }} />Percorri esta parte e quero marcar meu avanço</label></section>)}
