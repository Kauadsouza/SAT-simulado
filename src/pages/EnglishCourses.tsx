...
export function EnglishCourses() {
  ...
  return <div className="learn-page"><PageHeading eyebrow={`${variantMeta.label.toUpperCase()} · ${allCourses.length} RECURSOS GRATUITOS`} title={variant === 'british' ? 'Sua rota para estudar e viver em Oxford.' : 'Um caminho americano, do básico ao independente.'} text={variant === 'british' ? 'Siga a rota britânica por nível e use os materiais do British Council e Cambridge para ouvir, falar, ler, escrever e reconhecer o vocabulário do Reino Unido.' : 'Use a VOA ou o USA Learns como curso principal e complemente com pronúncia, gramática, leitura e expressões americanas.'} />
    ...
    <section><div className="learn-section-head"><div><span className="learn-eyebrow">CURSO ORGANIZADO · A1 → B2</span><h2>Uma rota clara, sem misturar variedades</h2></div><span className="learn-pill">{variantMeta.shortLabel}</span></div><div className="learn-roadmap">{courseRoadmapByVariant[variant].map((step, index) => <article className="learn-card" key={step.level}><span className="learn-step-number">0{index + 1}</span><small>{step.level}</small><h3>{step.title}</h3><p>{step.text}</p></article>)}</div></section>
    ...
    <p className="learn-source">Links oficiais conferidos em {RESEARCH_DATE}. O material indicado é gratuito; cadastro pode ser necessário em alguns cursos. Serviços extras podem ser pagos e esta seleção não promete certificado gratuito. Seu andamento aqui é manual e não é importado dos sites.</p>
  </div>;
}
